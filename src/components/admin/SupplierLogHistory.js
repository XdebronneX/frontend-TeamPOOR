...
const SupplierHistory = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { handleSubmit, register, reset, setValue, formState: { errors } } = useForm(); // useForm hook for form validation
  const [productInfo, setProductInfo] = useState([]);
  const { users } = useSelector((state) => state.allUsers);
  const { products: allProducts } = useSelector((state) => state.allProducts);
    ...
    <Fragment>
      ...
      <Box p={4}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} encType='multipart/form-data'>
          <Box display="flex" flexDirection="row" flexWrap="wrap" justifyContent="space-between">
            {/* Add form validation using register */}
            <FormControl mb={4} flex="1" mr={4}>
              <FormLabel htmlFor="supplier_field">Supplier</FormLabel>
              <Select
                id="supplier_field"
            ...
            <FormControl mb={4} flex="1" mr={4}>
              ...
              <Input
                type="date"
                id="dateDelivered_field"
                {...register('dateDelivered', { required: true })}
              />
              {errors.dateDelivered && <span>This field is required</span>}
            </FormControl>
          </Box>
          <Box display="flex" flexDirection="row" flexWrap="wrap" justifyContent="space-between">
            <FormControl mb={4} flex="1" mr={4}>
              ...
              <Input
                ...
                onChange={(e) => setInvoiceId(e.target.value)}
              />
              {errors.invoiceId && <span>This field is required</span>}
            </FormControl>
            <FormControl mb={4} flex="1">
              <FormLabel htmlFor="totalPrice_field">Total Price</FormLabel>
              <Input
          ...
          <FormControl mb={4} flex="1">
            ...
            {errors.productId && <span>Please choose a product</span>}
          </FormControl>
          <Table variant="striped" colorScheme="gray" size="md">
            <Thead>
              <Tr>
